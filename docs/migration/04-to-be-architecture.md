...
| 실시간 통신 | Supabase Realtime | Spring STOMP 기반 WebSocket |
...
다음 항목에 대해 사용자 리뷰와 동의를 완료하여 TO-BE 아키텍처를 1차 확정한다.

- [x] 전체 시스템과 모듈 책임
- [x] 통신 방식과 선택 이유
- [x] 핵심 트랜잭션과 commit 이후 작업의 구분
- [x] 채팅 저장·전달·복구 기준
- [x] 목표 논리 데이터 모델과 점진적 이전 원칙
- [x] 기술 결정 필요 항목의 처리 순서

다음 문서 `05-migration-roadmap.md`에서 도메인별 마이그레이션 순서, 사전 조건, 전환 기준과 롤백 조건을 작성한다. 구현 세부사항을 조사하며 중요한 결정이 변경되면 기존 내용을 덮어쓰지 않고 새로운 ADR로 변경 이유를 기록한다.
