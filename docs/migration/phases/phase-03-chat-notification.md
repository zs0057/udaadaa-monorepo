...
2026-08-06 Flutter 전환 준비 중 발견한 3개 갭 보완: (1) `post-initial-chat-data`/`stage-post-initial-chat-data` Edge Function 소스를 다시 조회해보니 지난번 코드 수정이 아직 재배포되지 않아 하드코딩된 `service_role` 키가 운영에 여전히 살아있는 것을 재확인 — 사용자가 Supabase CLI로 직접 재배포 진행 중. (2) `GET /rooms` 응답에 `myLastReadSequence`를 추가해, 방 목록 화면의 "안읽음 N명" 배지를 클라이언트가 계산할 수 있게 했다(기존엔 방별 마지막 메시지만 알 수 있었고 내가 어디까지 읽었는지는 알 방법이 없었음). (3) `GET /rooms/{roomId}/messages`에 Moderation 차단 필터링(`ModerationReader.canInteractWith`)과 개인 메시지 숨김(`blocked_messages`) 필터링을 적용했다 — 기존 `post-initial-chat-data`가 하던 필터링을 그대로 재현한 것으로, 이걸 안 하고 Flutter를 전환했으면 차단 기능이 사실상 무력화되는 회귀가 났을 것이다. 방 목록의 마지막 메시지 미리보기(`GET /rooms`의 `lastMessage`)는 기존 동작대로 필터링하지 않는다.
